...
class l10n_ar_wsafip_keygen_config(osv.osv_memory):
    ...
    def update_data(self, cr, uid, ids, company_id, context=None):
        ...
        v = {
            'wsafip_country_id': company.country_id.id if company.country_id
            else None,
            'wsafip_state_id': company.state_id.id if company.state_id
            else None,
            'wsafip_city': company.city
            or None,
            'wsafip_cuit': company.partner_id.vat[2:] if company.partner_id.vat
            else None,
        }
        return {'value': v}
    ...
    def execute(self, cr, uid, ids, context=None):
        ...
        for wzr in self.read(cr, uid, ids):
            pk_id = pairkey_obj.create(cr, uid, {'name': wzr['wsafip_name']},
                                       context=context)
            ...
            x509_name.serialNumber = 'CUIT %s' % \
                wzr['wsafip_cuit'].encode('ascii', 'ignore')
            x509_name.CN = wzr['wsafip_name'].encode('ascii', 'ignore')
            pairkey_obj.generate_keys(cr, uid, [pk_id], context=context)
            pairkey_obj.generate_certificate_request(cr, uid, [pk_id],
                                                     x509_name, context=context)
...
class l10n_ar_wsafip_loadcert_config(osv.osv_memory):
    ...
    def update_data(self, cr, uid, ids, certificate_id, context=None):
        v = {}
        if certificate_id:
            certificate = self.pool.get('crypto.certificate').browse(
                cr, uid, certificate_id)
            v = {
