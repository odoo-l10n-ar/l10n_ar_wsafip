...
class wsafip_server(osv.osv):
    ...
    _columns = {
        ...
        'class': fields.selection([('production', 'Production'),
                                   ('homologation', 'Homologation')], 'Class'),
        'url': fields.char('URL', size=512),
        'auth_conn_id': fields.one2many(
            'wsafip.connection', 'logging_id', 'Authentication Connections'),
        'conn_id': fields.one2many(
            'wsafip.connection', 'server_id', 'Service Connections'),
    }
